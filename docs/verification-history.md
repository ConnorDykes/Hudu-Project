...
These are historical checks from earlier development passes on September 10, 2026. Counts and revisions below describe those checkpoints, not the current release. See [current verification results](verification-results.md) for the submission revision, release artifacts, and current test totals.
